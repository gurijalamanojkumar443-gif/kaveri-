...
class KaveriAgentEngine:
    ...
    def process_message(
        ...
        """
        Main cognitive loop:
        1. Checks for active pending confirmations (e.g. Cancellation approval)
        ...
        if state.pending_action:
            ...
            if pending.action_type == "cancel_booking":
                affirmative_words = ["yes", "confirm", "proceed", "cancel it", "cancel booking", "sure", "ok", "yep", "do it", "please cancel"]
                negative_words = ["no", "cancel that", "don't", "keep it", "nevermind", "abort", "wait", "nope", "stop"]
        ...
        # Spending / Expense calculation intent
        ...
        # Room Search / Availability intent
        if any(w in lower_msg for w in ["find", "search", "available", "availability", "vacanc", "room", "book a", "rates"]):
            prop_val = None
            ...
            if not rooms:
                ...
                lines.append("Would you like to reserve one of these rooms or customize your travel dates?")
                reply = "\n".join(lines)
        ...
        if any(w in lower_msg for w in ["hello", "hi", "hey", "namaste", "good morning", "good evening"]):
            reply = (
                ...
                f"• 💳 **Calculating your total spending & receipts**\n"
